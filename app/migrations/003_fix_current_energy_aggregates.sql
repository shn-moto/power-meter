DROP MATERIALIZED VIEW IF EXISTS samples_monthly;
...
SELECT
    ...
    CASE
        WHEN count(*) <= 1 THEN 0::DOUBLE PRECISION
        ELSE avg(power_w) * LEAST(
            GREATEST(
                (
                    EXTRACT(EPOCH FROM max(captured_at) - min(captured_at))
...
CALL refresh_continuous_aggregate('samples_hourly', NULL, NULL);
CALL refresh_continuous_aggregate('samples_daily', NULL, NULL);
CALL refresh_continuous_aggregate('samples_monthly', NULL, NULL);